...
class Calculator implements T_Calculate {
  ...
  divide(firstNumber: number, secondNumber: number): number {
    try {
      if (secondNumber === 0)
        throw new Error(`You can't divide by 0. Enter a number other than 0`);
      else {
        return firstNumber / secondNumber;
      }
    } catch (error: any) {
      ...
      rdl.question('Enter first number:', (num1: string) => {
        rdl.question('Enter second number:', (num2: string) => {
          ...
          if (isNaN(number1) || isNaN(number2)) {
            console.log(
              `${number1} or ${number2} is not a number. Enter valid numbers only `,
            );
            recursiveQuestion();
            return;
          }
  ...
  userInput() {
    ...
    rdl.on('line', (input: string) => {
      if (parseInt(input) === 1) {
        ...
        this.menu.greet(
          `Hello! For validation purpose, you have to enter 2 inputs but only the first input is used for the Sqrt operation.`,
        );
